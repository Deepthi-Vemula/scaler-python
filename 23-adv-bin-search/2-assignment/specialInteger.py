...
def check(A, k, B):
    ...
    for i in range(k):
        sumVal += A[i]
    # print("\tbefore sumVal", sumVal)
    if sumVal > B:
        return False
    for i in range(k, n):
        sumVal = sumVal + A[i] - A[i-k]
        # print("\t", sumVal, A[i], A[i-k])
        if sumVal > B:
            return False
    return True

def getSplInteger(A, start, end, B):
    if start > end:
        return -1
    mid = int((start+end)/2) + 1
    val = check(A, mid, B)
    print("check val ", mid, val, start, end)
    if not val:
        return getSplInteger(A, start, mid-1, B)
    val1 = getSplInteger(A, mid+1, end, B)
    if val1 == -1:
        return mid
    return val1
...
class Solution:
    ...
    # @return an integer
    def solve(self, A, B):
        return getSplInteger(A, 0, len(A), B)
